...
public class Aggregate extends Operator {
	...
	// some code goes here
        if (gfield == Aggregator.NO_GROUPING) {
            ...
            fn[0] = aop.toString() + "(" + child.getTupleDesc().getFieldName(afield) + ")";
            TupleDesc td = new TupleDesc(tp, fn);
            return td;
         } else {
            Type tp[] = new Type[2];
            tp[0] = child.getTupleDesc().getFieldType(gfield);
            tp[1] = Type.INT_TYPE;
            String fn[] = new String[2];
            fn[0] = child.getTupleDesc().getFieldName(gfield);
            fn[1] = aop.toString() + "(" + child.getTupleDesc().getFieldName(afield) + ")";
            TupleDesc td = new TupleDesc(tp, fn);
            return td;
        }
    }

...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		//System.out.println("how get here");
		switch (pid.pgcateg()) {
...
//		System.out.println("ok");
		...
		int numSlot = page.getNumTuples();
		Iterator<Tuple> it = page.iterator();
		for (int i = 0; i < numSlot / 2; i++) {
			Tuple tuple = it.next();
			page.deleteTuple(tuple);
			newPage.insertTuple(tuple);
		}
		if (page.getLeftSiblingId() != null) {
			BTreePageId prevId = page.getLeftSiblingId();
			BTreeLeafPage prevPage = (BTreeLeafPage) getPage(tid, dirtypages, prevId, Permissions.READ_WRITE);
			prevPage.setRightSiblingId(newPage.getId());
		}
		newPage.setRightSiblingId(page.getId());
		newPage.setLeftSiblingId(page.getLeftSiblingId());
		page.setLeftSiblingId(newPage.getId());
		it = page.iterator();
		Tuple copyUp = it.next();
		BTreeEntry entry = new BTreeEntry(copyUp.getField(keyField), newPage.getId(), page.getId());
		BTreeInternalPage parentToInsert = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), entry.getKey());
		//page.setParentId(parentToInsert.getId());
		//newPage.setParentId(parentToInsert.getId());
		/*if (entry.getLeftChild().pageNumber() == 506 && entry.getRightChild().pageNumber() == 1) {
			Iterator<BTreeEntry> itt = parentToInsert.iterator();
			while (itt.hasNext()) {
				BTreeEntry tt = itt.next();
				System.out.println(tt.getLeftChild().pageNumber() + " " + tt.getRightChild().pageNumber());
			}
		}*/
		parentToInsert.insertEntry(entry);
		updateParentPointers(tid, dirtypages, parentToInsert);
		if (copyUp.getField(keyField).compare(Op.GREATER_THAN_OR_EQ, field)) {
			return newPage;
		} else {
			return page;
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Field pushUpField = pushUp.getKey();
		page.deleteKeyAndLeftChild(pushUp);
		pushUp = new BTreeEntry(pushUpField, newPage.getId(), page.getId());
		updateParentPointers(tid, dirtypages, newPage);
		updateParentPointers(tid, dirtypages, page);
		BTreeInternalPage parent = (BTreeInternalPage)getParentWithEmptySlots(tid, dirtypages, page.getParentId(), pushUp.getKey());
		//System.out.println(parent.getNumEmptySlots());
		//System.out.println(parent.getMaxEntries());
		parent.insertEntry(pushUp);
		updateParentPointers(tid, dirtypages, parent);
		//System.out.println(parent.getId() + " " + parent.getNumEmptySlots());

		if (pushUpField.compare(Op.GREATER_THAN_OR_EQ, field)) {
			return newPage;
		} else {
			return page;
		}
	...
	public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
		...
		//System.out.println(leafPage.getNumEmptySlots());

		leafPage.insertTuple(t);

		ArrayList<Page> dirtyPagesArr = new ArrayList<Page>();
		dirtyPagesArr.addAll(dirtypages.values());
		/*for (int i = 0; i < dirtyPagesArr.size(); i++) {
			//System.out.println(dirtyPagesArr.get(i).getId());
			if (dirtyPagesArr.get(i).getId().pageNumber() == 2) {
				System.out.println(((BTreePage)dirtyPagesArr.get(i)).getNumEmptySlots());
			}
		}

 	 	System.out.println(leafPage.getNumEmptySlots());*/
		//System.out.println(leafPage.getNumEmptySlots() + " " + ((BTreePage)(dirtyPagesArr.get(2))).getNumEmptySlots() + " " + leafPage.getId().pageNumber() + " " +(dirtyPagesArr.get(2)).getId().pageNumber());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numSibling = sibling.getNumTuples();
		int numCur = page.getNumTuples();
		int need = (numSibling + numCur) / 2 - numCur;
		Field key = null;
		if (!isRightSibling) {
	...
	private void handleMinOccupancyInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if(leftEntry != null) leftSiblingId = leftEntry.getLeftChild();
		if(rightEntry != null) rightSiblingId = rightEntry.getRightChild();
		//System.out.println("Dealing " + page.getId());
		...
		//	System.out.println("left = " + leftSiblingId);
			if(leftSibling.getNumEmptySlots() >= maxEmptySlots) {
		...
		//	System.out.println("right = " + rightSiblingId);

			BTreeInternalPage rightSibling = (BTreeInternalPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
			// if the right sibling is at minimum occupancy, merge with it. Otherwise
			// steal some entries from it
			if(rightSibling.getNumEmptySlots() >= maxEmptySlots) {
		//		System.out.println("choice merge");
				mergeInternalPages(tid, dirtypages, page, rightSibling, parent, rightEntry);
			}
			else {
		//		System.out.println("choice steal");
		//		System.out.println("Right has " + rightSibling.getNumEntries() + "entries");
		//		System.out.println("Left has " + page.getNumEntries() + "entries");

				stealFromRightInternalPage(tid, dirtypages, page, rightSibling, parent, rightEntry);
				rightSibling = (BTreeInternalPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
		//		System.out.println("Right has " + rightSibling.getNumEntries() + "entries");
		//		System.out.println("Left has " + page.getNumEntries() + "entries");
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
			...
			int need = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
			BTreeEntry entry = null;
			BTreePageId leftId = it.next().getRightChild();
			it = page.iterator();
			BTreePageId rightId = it.next().getLeftChild();
			BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftId, rightId);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int need = (rightSibling.getNumEntries() + page.getNumEntries()) / 2 - page.getNumEntries() - 1;
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//System.out.println(rightPage.getRightSiblingId());
		...
		//ystem.out.println(rightPage.getId());
		//System.out.println(leftPage.getRightSiblingId());

		BTreePageId nextPageId = rightPage.getRightSiblingId();
		if (nextPageId != null) {
			leftPage.setRightSiblingId(nextPageId);
			((BTreeLeafPage)getPage(tid, dirtypages, nextPageId, Permissions.READ_WRITE)).setLeftSiblingId(leftPage.getId());
		} else {
			leftPage.setRightSiblingId(null);
		}
		//System.out.println(leftPage.getId() + " " + nextPageId);
		//System.out.println(parent.getNumEntries());
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreePageId rightId = it.next().getLeftChild();
		it = leftPage.reverseIterator();
		BTreePageId leftId = it.next().getRightChild();

		BTreeEntry entry = null;
		/*for (; it.hasNext(); entry = it.next()) {
	...
	public ArrayList<Page> deleteTuple(TransactionId tid, Tuple t) 
		...
		// or merge with one of the siblings
		int maxEmptySlots = page.getMaxTuples() - page.getMaxTuples()/2; // ceiling
		//System.out.println(pageId + " ---- " + page.getNumEmptySlots() + " " + maxEmptySlots);
